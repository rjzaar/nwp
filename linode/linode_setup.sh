...
################################################################################
# linode_setup.sh - Set up Linode CLI and environment for NWP
################################################################################
#
# This script prepares your local machine to work with Linode by:
...
LINODE_DIR="$SCRIPT_DIR"
KEYS_DIR="$LINODE_DIR/keys"
CONFIG_DIR="$HOME/.nwp"
CONFIG_FILE="$CONFIG_DIR/linode.yml"
...
configure_linode_cli_interactive() {
    ...
    if confirm "Do you have your API token ready?" "y"; then
        ...
        echo "  • Default type → Enter: ${GREEN}g6-nanode-1${NC} (smallest/cheapest)"
...
setup_ssh_keys() {
    ...
    if [ -f "$HOME/.ssh/id_rsa.pub" ] || [ -f "$HOME/.ssh/id_ed25519.pub" ]; then
        ...
        if confirm "Generate a new SSH key pair for NWP?" "y"; then
            ssh-keygen -t ed25519 -C "nwp-deployment" -f "$HOME/.ssh/id_ed25519" -N ""
    ...
    # Create NWP-specific key if desired
    if [ ! -f "$KEYS_DIR/nwp_linode" ]; then
        echo ""
        print_info "You can create a dedicated SSH key pair for NWP Linode servers"
        print_info "This is recommended for better security (separate keys for different servers)"
        echo ""

        if confirm "Create a dedicated NWP Linode SSH key?" "y"; then
            ssh-keygen -t ed25519 -C "nwp-linode-deployment" -f "$KEYS_DIR/nwp_linode" -N ""
            chmod 600 "$KEYS_DIR/nwp_linode"
            chmod 644 "$KEYS_DIR/nwp_linode.pub"
            print_success "Dedicated NWP Linode key created at: $KEYS_DIR/nwp_linode"

            print_info "Your NWP Linode public key:"
            echo ""
            cat "$KEYS_DIR/nwp_linode.pub"
            echo ""
        ...
        print_success "NWP Linode SSH key already exists"
    fi
}
...
setup_ssh_config() {
    ...
    # Check if NWP config already exists
    if grep -q "# NWP Linode Servers" "$ssh_config"; then
        print_success "SSH config for NWP already exists"
    else
        print_info "Adding NWP configuration to ~/.ssh/config"
...
# NWP Linode Test Server
Host nwp-test
    HostName # Set this to your test server IP or domain
    User nwp

# NWP Linode Production Server
Host nwp-prod
    HostName # Set this to your production server IP or domain
    User nwp
EOF
...
create_config_files() {
    ...
    # Create linode.yml if it doesn't exist
...
verify_setup() {
    ...
    if [ -f "$KEYS_DIR/nwp_linode.pub" ]; then
        print_success "NWP Linode SSH key found"
        key_found=true
    fi
    ...
    if command -v jq &> /dev/null; then
        print_success "jq is installed (needed for deployment)"
    else
    ...
    if [ $errors -eq 0 ] && [ $warnings -eq 0 ]; then
        print_success "✓ All checks passed! You're ready to deploy to Linode."
        echo ""
        echo "${BOLD}Next steps:${NC}"
        echo "  1. Install jq: ${GREEN}sudo apt-get install jq${NC}"
        echo "  2. Upload StackScript: ${GREEN}./linode_upload_stackscript.sh${NC}"
        echo "  3. Create test server: ${GREEN}./linode_create_test_server.sh${NC}"
        return 0
...
display_next_steps() {
    ...
    echo "Your Linode environment is now set up! Here's what to do next:"
    echo ""
    echo "1. ${BOLD}Review and edit configuration:${NC}"
    echo "   nano $CONFIG_FILE"
    echo ""
    echo "2. ${BOLD}List available Linode plans:${NC}"
    echo "   linode-cli linodes types"
    echo ""
    echo "3. ${BOLD}List available regions:${NC}"
    echo "   linode-cli regions list"
    echo ""
    echo "4. ${BOLD}Create your first test server:${NC}"
    echo "   cd $LINODE_DIR"
    echo "   ./linode_provision.sh --label nwp-test --domain test.example.com"
    echo ""
    echo "5. ${BOLD}Read the documentation:${NC}"
    echo "   cat $LINODE_DIR/docs/SETUP_GUIDE.md"
    echo ""
...
# Main execution
main() {
    print_header "NWP Linode Setup"

    echo "This script will set up your environment for deploying NWP sites to Linode."
    echo ""
    echo "It will:"
