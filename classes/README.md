...
`lib/moodle-gate.sh` consults the class *only on the failure path*. An artifact
that passes the ordinary ops#137 scan passes regardless of class. So declaring a
class can never newly refuse a site that used to deploy, and can never silence
the evidence trail.
