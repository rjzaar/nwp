...
function local_browse_render_curated($context): void {
    ...
    // Rail palette (matches local_browse_rail_meta) so each intent tile is
    // colour-coded and the whole board reads tighter + coloured, not flat green.
    $palette = ['#5D4037', '#1565C0', '#7B1FA2', '#2E7D32', '#00838F', '#C62828', '#455A64'];
    ?>
    <p style="color:#555;font-size:1em;margin:0 0 14px;"><?php echo s(get_string('curated_intro', 'local_browse')); ?></p>
    <?php $ti = 0; foreach ($tiles as $tile):
        $courses = local_browse_courses_by_shortname($tile['courses']);
        if (empty($courses)) {
            continue; // Nothing resolves in this Moodle — skip the tile.
        }
        $c = $palette[$ti % count($palette)]; $ti++;
        // 12%-tint header background derived from the tile colour.
        $tint = $c . '14';
    ?>
        <div style="margin-bottom:16px;border:1px solid #e6e6e6;border-left:5px solid <?php echo $c; ?>;border-radius:8px;padding:12px 14px 14px;background:<?php echo $tint; ?>;">
            <h2 style="color:<?php echo $c; ?>;font-size:1.15em;margin:0 0 3px;line-height:1.2;"><?php echo s($tile['heading']); ?></h2>
            <?php if ($tile['blurb'] !== ''): ?>
                <p style="color:#555;margin:0 0 10px;font-size:0.92em;line-height:1.4;"><?php echo s($tile['blurb']); ?></p>
            <?php endif; ?>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:8px;">
                <?php foreach ($courses as $course) {
                    local_browse_course_card($course, $context, $c);
                } ?>
            </div>
        </div>
    <?php endforeach; ?>
...
function local_browse_render_browse($context): void {
    ...
    // Base params reflect the RESOLVED toggle state so the pill preserves the
    // content toggle (including a logged-in user's persisted preference).
    // (The v1-archive catalog toggle was removed 2026-07-21: v3 already contains
    // every v1 course, polished + 6 more, so the archive added only stale copies;
    // the archive site ss2 was retired.)
    $baseparams = ['view' => 'browse', 'content' => $content];
