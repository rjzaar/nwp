...
- **`servers/` split.** 80 tracked files carrying per-host IDENTITY — nginx
  ...
  **in place** by a private per-server repository (`servers/<host>/.git`, remote
  ...
  units). See [NWP-ADR-0039](docs/decisions/0039-instance-state-in-private-overlay-repos.md).
...
The v0.30.0 release is the **fresh-repo baseline**: F23 (Project Separation v2)
ships in full through phases 1–8 and 10, the prior `.git` history was retired
in favor of a clean tree, and project-specific recipes (mt, dir) now live
alongside their sites instead of in NWP core. See
`docs/proposals/F23-project-separation.md` and
`docs/proposals/F25-baseline-reset-cleanup.md` for the full story.

### Added — F23: Project Separation v2 (Phases 1-8, 10)
- **Per-site config files**: every site now has its own `sites/<name>/.nwp.yml`
  with `schema_version`, `nwp_version_created`, `nwp_version_updated`. Reads
  via the new layered resolver: `.nwp.local.yml` -> `.nwp.yml` -> global
  `nwp.yml`.
- **Schema migration framework** (`lib/migrate-schema.sh` + numbered scripts in
  `lib/migrations/{site,global,server}/`): each scope has an independent
  version counter; `_run_migrations` takes a pre-migration backup before each
  step and stamps `nwp_version_updated`.
- **`pl site` family**: `list`, `show`, `schema`, `migrate [--all]`,
  `init [--all]` for managing per-site configs.
- **`pl server` family**: `list`, `show`, `status [--all]`, `sites <name>`,
  `schema`, `migrate [--all]` for managing servers under
  `servers/<name>/.nwp-server.yml`.
- **`pl proposals`**: aggregates `docs/proposals/` across the NWP root and
  every site (`sites/*/docs/proposals/`); supports `--site=`, `--status=`,
  `--root`, `--sites` filters.
- **`pl doctor`** now checks per-site and per-server schema versions, and
  warns when any are stale or missing.
- **Project resolver** (`lib/project-resolver.sh`) and **server resolver**
  (`lib/server-resolver.sh`) auto-source via `lib/common.sh` so all scripts
  inherit `resolve_project`, `get_backup_dir`, `resolve_server`,
  `get_server_ip`, `get_server_ssh_command`, etc.

### Changed
- **Modules moved into sites** (Phase 2): `modules/<custom_module>/` ->
  `sites/<owning_site>/web/modules/custom/<custom_module>/`. The parent NWP
  repo no longer tracks Drupal custom modules.
- **Pipelines moved into sites** (Phase 3): `mt/`, `cathnet/`, `fin/` ->
  `sites/<name>/pipeline/`. Each pipeline is part of its site, not a
  separate root tree.
- **Backups moved into sites** (Phase 4): `sitebackups/<name>/` ->
  `sites/<name>/backups/`. The legacy location is still honored for
  unmigrated sites via `get_backup_dir()`'s fallback chain. All 24+ call
  sites updated.
- **Server infrastructure moved** (Phase 8): `email/` and `linode/` ->
  `servers/nwpcode/{email,linode}/`. The new `servers/nwpcode/` is its
  own git repo (committed locally as `fb37f6e`, not pushed). Scripts
  `setup.sh`, `stg2prod.sh`, `live.sh`, `stg2live.sh` updated to prefer
  the new path with legacy fallback.
- **`live.server_ip` removed** from per-site `.nwp.yml`: now resolved
  through `servers/<name>/.nwp-server.yml` via the server resolver. Sites
  reference servers by name only (`live.server: nwpcode`).
- **Parent `.gitignore`**: `/sites/*` (except `.gitkeep`) and `/servers/*`
  (except `.gitkeep`) are gitignored — production sites (avc, ss) keep
  their own repos; experimental sites and servers stay filesystem-only
  until explicitly initialized.

### Migration Notes
- Run `pl site list` to see schema status of every site.
- Run `pl site migrate <site>` to upgrade an individual site's `.nwp.yml`
  if its `schema_version` lags `pl site schema`.
- The legacy `nwp.yml linode.servers.nwpcode.*` block is still readable as
  a fallback in `get_server_config()`, but the canonical source is now
  `servers/nwpcode/.nwp-server.yml`.
- F23 Phase 9 (OAuth2/Guild sync) and F24 (unified backup strategy)
  remain on the roadmap and are not part of this release.

### Added — F25: Baseline Reset Cleanup
- Project-specific recipes (`mt`, `dir`) extracted to `sites/<name>/recipe.yml`;
  `lib/install-common.sh` now resolves them via a fallback in `recipe_exists()`,
  `get_recipe_value()`, `get_recipe_list_value()`, and `list_recipes()`.
- F23 v1 archived to `docs/archive/F23-project-separation-v1-superseded.md`;
  v2 promoted to the canonical `docs/proposals/F23-project-separation.md`.
- Solveit methodology proposal converted from `.docx` to
  `docs/proposals/F26-solveit-methodology.md`.

### Removed — F25: Baseline Reset Cleanup
- `scripts/commands/fin-monitor.sh` wrapper (the standalone fin project ships
  its own monitor scripts) and the `fin) echo "utility"` branch in
  `scripts/commands/site.sh::_infer_project_type()`.
- Session-recovery dumps `HANDOVER-P50.md`, `IMPLEMENT-P50.md`, the one-time
  `migrate-verification-v2.sh` script, and the redundant `.docx` reports under
  `docs/reports/`.
