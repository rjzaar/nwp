...
`<mayo-domain>` has been migrated from vanilla Open Social to the AVC Drupal distribution in-place at `$HOME/nwp/sites/mayo/dev/`. It has a working `mayo_content` module (13 pages, 10 groups, menus, footer block), 35 sanitized users, and v2 `.nwp.yml` layout. However:
...
1. **`mayo/mayo` private git repo** (new, on `<gitlab-host>`) — site template, mirroring the `nwp/avc-project` pattern; `mayo_content` module lives inside.
...
7. **verifier handoff via `ops/verifier-log` issues** — CI posts "Deploy ready"; verifier polls, verifies minisign, deploys.
...
- Mayo code, content, and fixtures are all backed up to `<gitlab-host>`.
...
- verifier can deploy that tarball to mayo1 with zero changes to real user data.
...
- `$HOME/nwp/sites/mayo/dev/` — working AVC site, DDEV project `mayo-dev`, 13 pages, 10 groups, 35 sanitized users. Not in any git repo.
- `$HOME/nwp/sites/mayo/dev/html/modules/custom/mayo_content/` — `mayo_content.install` (~700 lines) with idempotent `_mayo_content_create_*` helpers.
- `$HOME/nwp/sites/mayo/dev/scripts/seed-sanitized-users.sh` — dev/stg-only user seeder (refuses to run against other DDEV projects).
- `$HOME/nwp/sites/mayo/.nwp.yml` + `dev/.nwp.yml` — v2 schema, recipe: avc, live target: mayo1.
- `$HOME/nwp/sites/mayo/stg/.nwp.yml` — stub.
- `$HOME/nwp/lib/sanitizers/mayo.sh` — exists, needs file-system layer verification.
- `$HOME/nwp/servers/mayo1/` — mayo1 infra tracked in NWP (scripts, systemd units, nginx configs).
- `git@<gitlab-host>:mayo/mayo.git` — empty, bootstrap proven 2026-04-08 (per memory).
- `git@<gitlab-host>:mayo/mayo-fixtures.git` — does not exist yet.
- `$HOME/nwp/.gitlab-ci.yml` — NWP tooling pipeline (not per-site); mayo pipeline will be separate.
- `$HOME/nwp/lib/minisign.sh` — signing primitives, already in place.
- `$HOME/nwp/scripts/ci/create-preview.sh`, `cleanup-preview.sh` — review app helpers to reuse.
- `$HOME/nwp/scripts/commands/build.sh`, `publish.sh` — `pl build` / `pl publish` implementations.
- `$HOME/nwp/servers/mayo1/scripts/verifier-deploy.sh` — consumer contract; dictates tarball naming and Package URL layout.
...
1.10. `git remote add origin git@<gitlab-host>:mayo/mayo.git && git push -u origin main`.
...
2.5. Seed from `$HOME/nwp/sites/mayo/backups/mayo-avc-fresh.sql`: gzip → `db/mayo-db-bootstrap-<timestamp>.sql.gz`, sign with `minisign -S -s ~/.minisign/nwp-deploy.key`, update `manifest.yml`, create `db/latest` symlink.
...
2.8. `git remote add origin git@<gitlab-host>:mayo/mayo-fixtures.git && git push -u origin main`.
...
2.11. On mirror-store: `ssh-keygen -t ed25519 -f ~/.ssh/mayo-fixtures-read -C "mirror-store fixtures consumer"`; register as **read-only** deploy key.
2.12. Copy `keys/pubkey.minisign` from the repo to `~/.config/mayo-fixtures.pub` on mirror-store (this is the real trust anchor; the in-repo copy is for bootstrap only).

### Phase 3 — Modernise `mayo_content` (add deploy hooks)

3.1. Edit `$HOME/nwp/sites/mayo/dev/composer.json` to require: `"drupal/config_ignore": "^3.3"`, `"drupal/config_filter": "^2.6"`, `"drupal/update_helper": "^4"`. Run `ddev composer update`.
3.2. Create `$HOME/nwp/sites/mayo/dev/html/modules/custom/mayo_content/mayo_content.deploy.php` with one function:
```php
...
4.1. Create `$HOME/nwp/sites/mayo/dev/config/sync/config_ignore.settings.yml` (at project level, not under `html/`, so it survives blue/green slot swaps). Ignored entries:
```
- system.site:mail
- system.site:name
- system.site:slogan
...
- search_api.server.*
```
4.2. Edit `$HOME/nwp/sites/mayo/dev/html/sites/default/settings.php`:
...
4.3. Create `$HOME/nwp/servers/mayo1/scripts/stamp-deployment-identifier.sh`: writes `<?php $settings['deployment_identifier'] = '<tarball-version>';` into the active slot's `settings.deploy.php`. Invoked by `bluegreen-swap.sh` **before** the symlink flip.
4.4. Patch `$HOME/nwp/servers/mayo1/scripts/bluegreen-swap.sh` to call `stamp-deployment-identifier.sh` pre-flip and `drush deploy` post-flip.

### Phase 5 — Build out `sites/mayo/stg/`

5.1. Create `$HOME/nwp/sites/mayo/stg/.ddev/config.yaml` — `name: mayo-stg`, `type: drupal10`, same PHP/DB/composer versions as dev, `ENV_TYPE=staging`.
...
5.5. Create `$HOME/nwp/sites/mayo/scripts/restore-sanitized-fixture.sh`: takes a path or GitLab Package URL, runs `ddev import-db`, `ddev drush deploy`, `ddev drush cr`. Refuses to run unless DDEV project is `mayo-dev` or `mayo-stg`.
5.6. Run `restore-sanitized-fixture.sh` against the fixtures repo's `db/latest` (cloned via the mirror-store read deploy key from Phase 2).
...
6.2. Create `$HOME/nwp/sites/mayo/dev/.gitlab-ci.yml` (becomes root of `mayo/mayo`). Uses `include:` from the templates project.
6.3. Pipeline stages (11):

| # | Stage | Runner tag | Script | Secrets |
|---|---|---|---|---|
| 1 | `verify:signature` | `mirror-store` | `git verify-commit HEAD` against runner-baked keyring | commit-signing pubkey ring |
| 2 | `lint` | `ai-host` | `composer validate --strict`, `php -l`, `phpcs --standard=Drupal` | none |
| 3 | `test:fixture` | `mirror-store` | `scripts/ci/fetch-mayo-fixture.sh` → verify → `scripts/ci/build.sh` → `ddev import-db` → `drush deploy` | `MAYO_FIXTURES_DEPLOY_TOKEN`, `NWP_DEPLOY_PUBKEY` |
| 4 | `test:phpunit` | `mirror-store` | `ddev drush test:run mayo_content` + PHPUnit kernel tests | none |
| 5 | `security:scan` | `nwp` | Reuse root pipeline's composer audit + secret detection block | none |
| 6 | `build` | `mirror-store` (protected) | `pl build mayo --tag $CI_COMMIT_SHORT_SHA` (unsigned) | none — rules gate: `$CI_COMMIT_TAG =~ /^v/` |
| 7 | `sign` | `mirror-store` (protected, dedicated runner, concurrency 1) | `minisign_sign out/mayo-*.tar.gz`, shred key after | `MINISIGN_SECRET_KEY` (file), `MINISIGN_PASSWORD` |
| 8 | `publish:package` | `mirror-store` (protected) | `pl publish mayo --file out/mayo-*.tar.gz` → GitLab Packages at `mayo%2Fmayo/packages/generic/mayo-deploy/<version>/`; post `Deploy ready: mayo <version>` issue to `ops/verifier-log` | `GITLAB_API_TOKEN_MAYO_PUBLISH`, `OPS_MONS_LOG_WRITE_TOKEN` |
| 9 | `deploy:review` | `mirror-store` | `scripts/ci/create-preview.sh mr-$CI_MERGE_REQUEST_IID-mayo`; dotenv `PREVIEW_URL` via `environment.url` | `MAYO_FIXTURES_DEPLOY_TOKEN` |
| 10 | `e2e` | `mirror-store` | Behat / Drupal Test Traits against `$PREVIEW_URL` | none |
| 11 | `cleanup:review` | `mirror-store` | `scripts/ci/cleanup-preview.sh mr-$CI_MERGE_REQUEST_IID-mayo`; `auto_stop_in: 1 week` | none |
...
6.5. Create `$HOME/nwp/scripts/ci/fetch-mayo-fixture.sh` (new): curl latest fixture from `mayo-fixtures` Package registry + signature, verify with `minisign_verify`, SQL sanity check (`zcat | head -c 1024 | grep -q 'CREATE TABLE'`), refuse on failure.
6.6. Register a dedicated mirror-store runner `mirror-store-sign` with `tags: [mirror-store, sign]`, concurrency 1, protected-only. Install `minisign` binary on it.
...
7.1. SSH to mayo1. Verify `$HOME/nwp/lib/sanitizers/mayo.sh` covers: DB (all real user data stripped), public files (WWCC scans, real photos replaced with placeholders), private files (documents, incident reports excluded entirely).
7.2. Patch gaps found in 7.1 — **each patch is a separate MR, human-reviewed, no AI merge**.
7.3. Create `$HOME/nwp/servers/mayo1/scripts/fixtures-publish.sh`: runs sanitizer, produces 3 tarballs (db/files-public/files-private-sanitized), signs each with minisign, clones `mayo/mayo-fixtures` via write deploy key, commits atomically (manifest + artifacts + `latest` symlinks), pushes.
7.4. Create `$HOME/nwp/servers/mayo1/systemd/mayo-fixtures-publish.service` and `.timer`. Timer runs nightly 02:00 Australia/Melbourne.
...
7.7. Back on dev/mirror-store: verify `git pull` in `~/nwp/work/mayo-fixtures` pulls the new snapshot; verify `minisign -V` passes.
...
8.4. Verify signed tarball at `https://<gitlab-host>/mayo/mayo/-/packages` — expect `mayo-deploy/v0.1.0/mayo-v0.1.0.tar.gz` and `.minisig`.
8.5. Verify `ops/verifier-log` has issue `Deploy ready: mayo v0.1.0` with paste-ready `verifier-deploy.sh mayo v0.1.0`.
8.6. Power up verifier, bring up `wg-verifier`, run `verifier-deploy.sh mayo v0.1.0`.
...
8.8. `verifier-say "deploy mayo v0.1.0 complete"`.
8.9. Close `Deploy ready` issue in `ops/verifier-log`.
...
- `servers/mayo1/scripts/verifier-deploy.sh` — consumer contract (lines 100–107 + 210–240)

---

## 6. Verification

- `cd ~/nwp/sites/mayo/dev && git log --oneline` shows signed commits. `git remote -v` points at `<gitlab-host>:mayo/mayo.git`.
...
- On mirror-store: `minisign -V -p ~/.config/mayo-fixtures.pub -m db/latest` returns "Signature and comment signature verified".
- `sites/mayo/stg/` running DDEV project `mayo-stg` with imported sanitized fixture; stg shows 13 pages, 10 groups.
- Pipeline green on `mayo/mayo v0.1.0` tag. Signed tarball in GitLab Packages. `ops/verifier-log` issue created.
- verifier deploy succeeds on mayo1. Prod site loads. `SELECT COUNT(*) FROM users_field_data` on prod unchanged.
- `verifier-say` confirmation reaches dev session.
...
2. `mayo/mayo v0.1.0` is deployed to mayo1 via verifier with zero changes to real user data.
...
(actual hostnames for the `mirror-store`, `ai-host`, and `verifier`,
the `<gitlab-host>` domain, the `<mayo-domain>` production domain, the
specific Mayo Studios deployment context, and milestone-to-commit-hash
mapping) live in the private instance addendum at
`nwp-instances/_proposals-private/F25-instance.md`.
