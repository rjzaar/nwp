...
backup_exists() {
    ...
    # F23 Phase 4: backups live at sites/<name>/backups/ with
    # legacy sitebackups/<name>/ still honored for unmigrated sites.
    local new_dir="$VERIFY_PROJECT_ROOT/sites/$site_name/backups"
    local legacy_dir="$VERIFY_PROJECT_ROOT/sitebackups/$site_name"
    local dir=""
    if [[ -d "$new_dir" ]]; then
        dir="$new_dir"
