...
10.2. **Course pack downloads:** Public page on example.com listing available course packs with download links. Each pack is a signed tar.gz containing JSON files + import instructions for any Moodle instance.
...
**Verification:**
- [ ] At least 5 course packs downloadable from example.com
...
|---|---|
| `nwpcom.org` — canonical AVC + governance | `example.com` (Phase 1+) / `avc.<example-prod-domain>` (today) |
...
**Phase 1 — Test consolidation.** Move the AVC test site from `avc.<example-prod-domain>` to `example.com` (root domain). `ss.<example-prod-domain>` stays. `<gitlab-host>` stays. Goal: prove the canonical AVC profile on a clean root domain before the prod cut. F30 manifests still reference `example.com` / `saint.school` (logical names) — only DNS/server config changes.
...
- **F26 (OIDC):** Multi-domain OIDC is required for federation. F26 must support multiple issuer/client domains and survive the `example.com` → `nwpcom.org` issuer rename. Every OIDC client config (Moodle plugin, member-site auth) needs a config update at Phase 2.
...
The biggest risk is OIDC issuer rename mid-flight. Mitigation: stand up `nwpcom.org` OIDC alongside `example.com` OIDC (dual-issue) for the cutover window, retire `example.com` issuer only after every member has migrated.
