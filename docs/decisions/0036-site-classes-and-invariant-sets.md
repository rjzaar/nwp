...
Mechanically, in `lib/moodle-gate.sh`, the class touches the gate at two
points. **Retarget** (before the scan): a `posture: local` declaration that
fully validates — right site, class permits `local`, obligations hold —
changes *which* class the artifact must delegate to, never *whether*; an
artifact delegating to the wrong source is refused, deliberately, so this is
the one class change that can newly refuse an artifact that used to pass.
**Exemption** (after the scan, on failure only): `none-stored` plus intact
evidence reclassifies the failure, loudly and ledgered. For every other
declaration — including an invalid, contradictory or mis-sited one — the gate
behaves exactly as if no class existed, and nothing the class says can
*silence* the scan (the evidence trail is intact either way).
...
- ops#103 — rgs has no canonical buildable source in any repo with a remote (its
  build tree lives only on the ai-host (`~/nwp/sites/rgs`));
