...
# release) and WHERE FROM (resolved source dir, its origin rung, and — when the
# source is tracked content of a git checkout — the checkout's HEAD commit).
# The deploy verb is the only honest writer: only it knows which resolved
# source dir actually went over the wire (ops#137: assert one tree, ship the
# same tree — this records that same tree).
#
# The lockfile is a CLAIM. The box is the fact (same rule as the rollback
# ledger); `pl moodle plugin drift` is the verb that measures the box. What the
# lockfile adds is the one thing the box cannot answer: provenance — which
# canonical commit the deployed bytes came from.
################################################################################

# yq resolver (mirrors _mp_yq; local so this lib stands alone in tests).
_md_yq() {
    if declare -F _mp_yq >/dev/null 2>&1; then _mp_yq; return $?; fi
    if command -v yq >/dev/null 2>&1; then echo yq
    elif [ -x "$HOME/.local/bin/yq" ]; then echo "$HOME/.local/bin/yq"
    else return 1; fi
}

# moodle_plugin_release_dir <plugin_dir> — echo $plugin->release of the plugin
# whose ROOT is this directory. FAILS (non-zero, no output) when the release
# line is absent or empty — the format_tabbed empty-release shape must be a
# distinct answer, never an empty success.
moodle_plugin_release_dir() {
    local f="${1%/}/version.php"
    [ -f "$f" ] || return 1
    local r
    r=$(sed -nE "s/.*\\\$plugin->release[[:space:]]*=[[:space:]]*['\"]([^'\"]+)['\"].*/\1/p" "$f" | head -1)
    [ -n "$r" ] || return 1
    printf '%s' "$r"
}

# moodle_plugin_source_provenance <plugin_dir> — which commit did this source
# dir come from?  Echoes exactly one of:
#   none                          not inside a git repo, OR inside one that does
#                                 not TRACK it (the upstream-Moodle-clone shape,
#                                 where the custom plugin is untracked files)
#   <sha> clean [<origin-url>]    tracked, worktree clean under the plugin dir
#   <sha> dirty [<origin-url>]    tracked, uncommitted changes under the dir
#
# The tracked-check is load-bearing: `rev-parse HEAD` alone inside a Moodle
# clone (or under a repo that gitignores sites/) would launder the HOST repo's
# HEAD as the plugin's provenance — a well-formed lie.
moodle_plugin_source_provenance() {
    local dir="${1%/}"
    local sha tracked
    sha="$(git -C "$dir" rev-parse HEAD 2>/dev/null)" || { echo "none"; return 0; }
    tracked="$(git -C "$dir" ls-files -- . 2>/dev/null | head -1)"
    [ -n "$tracked" ] || { echo "none"; return 0; }
    local state="clean"
    [ -n "$(git -C "$dir" status --porcelain -- . 2>/dev/null)" ] && state="dirty"
    local origin
    origin="$(git -C "$dir" remote get-url origin 2>/dev/null || true)"
    if [ -n "$origin" ]; then printf '%s %s %s' "$sha" "$state" "$origin"
    else printf '%s %s' "$sha" "$state"; fi
}

# moodle_lock_record <lockfile> <site> <tier> <plugin> <src_dir> <origin>
# Upsert one deployed-plugin entry. FAIL-CLOSED: an unreadable source version,
# or no yq, refuses (no literal substituted for a measurement not taken).
# A missing release records null — a real state (format_tabbed), not a failure.
moodle_lock_record() {
    local lockfile="$1" site="$2" tier="$3" plugin="$4" src_dir="$5" origin="$6"
    local yq_bin
    yq_bin="$(_md_yq)" || { _md_err "moodle_lock_record: yq not found — cannot write ${lockfile}"; return 1; }
    local version
    version="$(moodle_plugin_version_dir "$src_dir")" || {
        _md_err "moodle_lock_record: no readable \$plugin->version in ${src_dir} — refusing to record ${plugin}."
        return 1
    }
    local release=""
    release="$(moodle_plugin_release_dir "$src_dir" 2>/dev/null || true)"
    local prov commit="" dirty=""
    prov="$(moodle_plugin_source_provenance "$src_dir")"
    if [ "$prov" != "none" ]; then
        commit="${prov%% *}"
        case "$prov" in *" dirty"*) dirty="true" ;; *) dirty="false" ;; esac
    fi
    mkdir -p "$(dirname "$lockfile")" || return 1
    [ -f "$lockfile" ] || printf '# Generated by `pl moodle plugin deploy --apply` (ops#73). A CLAIM, not the box.\n' > "$lockfile"
    LOCK_SITE="$site" LOCK_TIER="$tier" LOCK_PLUGIN="$plugin" \
    LOCK_VERSION="$version" LOCK_RELEASE="$release" LOCK_ORIGIN="$origin" \
    LOCK_SRC="$src_dir" LOCK_COMMIT="$commit" LOCK_DIRTY="$dirty" \
    LOCK_AT="$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    "$yq_bin" eval -i '
        .lockfile_version = 1 |
        .site = strenv(LOCK_SITE) |
        .tiers[strenv(LOCK_TIER)][strenv(LOCK_PLUGIN)] = {
            "version":       strenv(LOCK_VERSION),
            "release":       (strenv(LOCK_RELEASE) | select(. != "") // null),
            "source_origin": strenv(LOCK_ORIGIN),
            "source_dir":    strenv(LOCK_SRC),
            "repo_commit":   (strenv(LOCK_COMMIT) | select(. != "") // null),
            "repo_dirty":    null,
            "deployed_at":   strenv(LOCK_AT)
        }' "$lockfile" || { _md_err "moodle_lock_record: yq write failed for ${lockfile}"; return 1; }
    if [ -n "$dirty" ]; then
        LOCK_TIER="$tier" LOCK_PLUGIN="$plugin" LOCK_DIRTY="$dirty" \
        "$yq_bin" eval -i \
            '.tiers[strenv(LOCK_TIER)][strenv(LOCK_PLUGIN)].repo_dirty = (strenv(LOCK_DIRTY) == "true")' \
            "$lockfile" || { _md_err "moodle_lock_record: yq write failed for ${lockfile}"; return 1; }
    fi
    return 0
}

# moodle_lock_report <lockfile> <site> — human-readable "what does pl claim is
# deployed". A missing lockfile is NOT RECORDED, non-zero — never "no drift".
moodle_lock_report() {
    local lockfile="$1" site="${2:-?}"
    if [ ! -f "$lockfile" ]; then
        _md_err "NOT RECORDED — no pl-managed plugin deploy has written a lockfile for '${site}' yet."
        _md_info "It is created by: pl moodle plugin deploy ${site} <plugin> --tier=live --apply"
        return 1
    fi
    local yq_bin
    yq_bin="$(_md_yq)" || { _md_err "cannot read ${lockfile}: yq not found"; return 1; }
    _md_info "Plugin lockfile: ${lockfile}"
    local tier plugin
    for tier in $("$yq_bin" eval '.tiers | keys | .[]' "$lockfile" 2>/dev/null); do
        printf '  tier %s:\n' "$tier"
        while IFS= read -r plugin; do
            [ -n "$plugin" ] || continue
            local v r c d o at
            v=$(T="$tier" P="$plugin" "$yq_bin" eval '.tiers[env(T)][env(P)].version'       "$lockfile")
            r=$(T="$tier" P="$plugin" "$yq_bin" eval '.tiers[env(T)][env(P)].release'       "$lockfile")
            c=$(T="$tier" P="$plugin" "$yq_bin" eval '.tiers[env(T)][env(P)].repo_commit'   "$lockfile")
            d=$(T="$tier" P="$plugin" "$yq_bin" eval '.tiers[env(T)][env(P)].repo_dirty'    "$lockfile")
            o=$(T="$tier" P="$plugin" "$yq_bin" eval '.tiers[env(T)][env(P)].source_origin' "$lockfile")
            at=$(T="$tier" P="$plugin" "$yq_bin" eval '.tiers[env(T)][env(P)].deployed_at'  "$lockfile")
            [ "$r" = "null" ] && r="(no release line)"
            local cshow="(no repo commit — source was not a tracked git checkout)"
            if [ "$c" != "null" ] && [ -n "$c" ]; then
                cshow="${c:0:12}"
                [ "$d" = "true" ] && cshow="${cshow} (DIRTY at deploy time)"
            fi
            printf '    %-28s version %-12s release %-18s commit %s\n' "$plugin" "$v" "$r" "$cshow"
            printf '    %-28s from %s  at %s\n' "" "$o" "$at"
        done < <(T="$tier" "$yq_bin" eval '.tiers[env(T)] | keys | .[]' "$lockfile" 2>/dev/null)
    done
    _md_info "The lockfile is a claim written at deploy time; the box is the fact."
    _md_info "Measure the box: pl moodle plugin drift ${site}"
    return 0
}
