#!/bin/bash
################################################################################
# lib/moodle-promote.sh — Moodle promotion SUBSTRATE (NWP-ADR-0031 D8 / ops D)
#
...
#     `canonical: live` site holds real students' records (NWP-ADR-0031 plane 5b);
...
moodle_write_config() {
    ...
    if ! _mp_is_noncanonical_tier "$tier"; then
        ...
        _mp_info "students' records (NWP-ADR-0031 plane 5b) and is never rewritten by the pipeline."
        return 1
    fi
    ...
    (
        umask 077
        cat > "$tmp" <<PHPEOF
<?php  // Moodle configuration file — GENERATED by NWP moodle_write_config (NWP-ADR-0031 D8).
...
# GENERATED by NWP moodle_generate_vhost (NWP-ADR-0031 D8) — tier: ${tier}
...
# GENERATED by NWP moodle_oauth_consumer_config (NWP-ADR-0031 D8 / F26) — DESCRIPTOR.
...
moodle_oauth_provider_snippet() {
    ...
    cat > "$out_file" <<YAMLEOF
# GENERATED by NWP moodle_oauth_provider_snippet (NWP-ADR-0031 D8 / F26) — ARTIFACT.
...
moodle_generate_oidc_apply_script() {
    ...
    cat > "$out_file" <<PHPEOF
<?php
// GENERATED by NWP moodle_generate_oidc_apply_script (NWP-ADR-0031 D8 / F26).
