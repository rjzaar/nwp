...
# F23 Phase 4: backups now live in sites/<name>/backups/, with legacy fallback.
_remaining_backup_dir=$(get_backup_dir "$SITENAME" 2>/dev/null || echo "")
if [ -n "$_remaining_backup_dir" ] && [ -d "$_remaining_backup_dir" ]; then
