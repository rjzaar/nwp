...
setup() {
    ...
    # F23 Phase 8: email scripts moved to servers/nwpcode/email/. Legacy fallback retained.
    ADD_SITE_EMAIL="${PROJECT_ROOT}/servers/nwpcode/email/add_site_email.sh"
    [ -f "$ADD_SITE_EMAIL" ] || ADD_SITE_EMAIL="${PROJECT_ROOT}/email/add_site_email.sh"
    SETUP_EMAIL="${PROJECT_ROOT}/servers/nwpcode/email/setup_email.sh"
    [ -f "$SETUP_EMAIL" ] || SETUP_EMAIL="${PROJECT_ROOT}/email/setup_email.sh"
