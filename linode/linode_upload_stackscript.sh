...
################################################################################
# linode_upload_stackscript.sh - Upload NWP StackScript to Linode
################################################################################
#
# This script uploads the linode_server_setup.sh as a StackScript to your
# Linode account so it can be used for provisioning servers.
#
# Usage:
#   ./linode_upload_stackscript.sh [OPTIONS]
#
# Options:
#   --update         Update existing StackScript instead of creating new
#   --label LABEL    Custom label (default: NWP Server Setup)
...
STACKSCRIPT_FILE="$SCRIPT_DIR/linode_server_setup.sh"
...
print_header "Upload NWP StackScript to Linode"

# Check if linode-cli is installed
if ! command -v linode-cli &> /dev/null; then
    print_error "Linode CLI is not installed"
    print_info "Run ./linode_setup.sh first to install and configure Linode CLI"
    exit 1
fi
...
if [ "$UPDATE_MODE" = true ]; then
    ...
    linode-cli stackscripts update "$STACKSCRIPT_ID" \
        --label "$LABEL" \
        --description "Automated NWP server provisioning with LEMP stack and security hardening" \
...
CONFIG_DIR="$HOME/.nwp"
mkdir -p "$CONFIG_DIR"
echo "$STACKSCRIPT_ID" > "$CONFIG_DIR/stackscript_id"
...
echo "    --label \"nwp-test-01\" \\"
echo "    --region us-east \\"
echo "    --type g6-standard-2 \\"
echo "    --image linode/ubuntu24.04 \\"
echo "    --root_pass \"SecurePassword123!\" \\"
echo "    --stackscript_id $STACKSCRIPT_ID \\"
echo "    --stackscript_data '{\"ssh_user\":\"nwp\",\"ssh_pubkey\":\"YOUR_PUBLIC_KEY\",\"hostname\":\"test.example.com\",\"email\":\"admin@example.com\"}'"
echo ""
print_success "StackScript ID saved to: $CONFIG_DIR/stackscript_id"
