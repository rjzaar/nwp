# NWP Proposals

This directory contains proposal documents for NWP features and enhancements.

## Proposal Status Overview

### Completed (see [milestones.md](../reports/milestones.md))
- **P01-P35** - Core phases 1-5c (Foundation through Live Deployment)
- **P50** - Unified Verification System
- **P51** - AI-Powered (Functional) Verification
- **P53** - Verification Categorization & Badge Accuracy
- **P54** - Verification Test Infrastructure Fixes
- **P55** - Opportunistic Human Verification
- **P56** - Production Security Hardening
- **P57** - Production Caching & Performance
- **P58** - Test Command Dependency Handling
- **P60** - (post-P58 follow-up; see milestones.md)
- **F03** - Visual Regression Testing
- **F04** - Distributed Contribution Governance (phases 1-5 complete)
- **F05** - Security Headers & Hardening
- **F07** - SEO & Search Engine Control
- **F09** - Comprehensive Testing Infrastructure
- **F12** - Unified Todo Command
- **F13** - Timezone Configuration
- **F14** - Claude API Integration
- **F15** - SSH User Management
- **F23** - Project Separation (phases 1–8, 10 complete; phase 9 deferred)
- **F25** - Pre-Baseline Cleanup (the work that produced this v0.30.0 baseline)

### Numbering note: there is no P59
The proposal numbering jumps from **P58** straight to **P60** by design.
P59 was never created — the slot was skipped during planning when the
verification follow-up work merged into P60. Please leave the gap; do not
recycle the number, as cross-references throughout the codebase assume the
existing IDs are stable.

### Rejected
- ~~**P52** - Rename NWP to NWO~~ — **REJECTED** (NWP is the permanent project name).
  Kept in `docs/proposals/` rather than archived so the rejection is visible
  to anyone scanning the proposal index.

### In Progress
- **F20** - SS Faith Formation Mobile & Desktop App (Phases 1-7 complete, 8-9 pending)

### Proposed
- **F21** - CathNet Neo4j/RDF Knowledge Graph with NLP-Driven QA
- **F22** - Claude Code Web Access — Remote CLI via Drupal
- **F26** - Integrating the SolveIt Methodology into NWP

### Pending (see [roadmap.md](../governance/roadmap.md))
- **F04** - Phases 6-8 (Issue Queue, Multi-Tier, Security Review)
- **F10** - Local LLM Support & Privacy Options
- **F11** - Developer Workstation Local LLM Configuration

### Possible (deprioritized, see [roadmap.md](../governance/roadmap.md))
- **F01** - GitLab MCP Integration
- **F02** - Automated CI Error Resolution
- **F06** - Malicious Code Detection Pipeline
- **F08** - Dynamic Cross-Platform Badges
- **X01** - AI Video Generation Integration

## Proposal Designation System

| Prefix | Meaning | Example |
|--------|---------|---------|
| **P##** | Core Phase Proposals | P01-P35 (complete), P50-P58 (complete, P52 rejected) |
| **F##** | Feature Enhancements | F01-F26 (11 complete, 1 in progress, 3 proposed, 2 pending, 4 possible) |
| **X##** | Experimental/Outlier | X01 (possible) |

## Lifecycle

1. **PROPOSED** - Idea documented, awaiting discussion
2. **PLANNED** - Accepted, scheduled for implementation
3. **IN PROGRESS** - Active development
4. **COMPLETE** - Implemented and verified
5. **Moved to milestones.md** - Archived as completed work

Last Updated: 2026-04-07
