...
A depthcontent activity's `content_json` (depths + youtube + question sets) **decomposes into typed atoms** (NWP-ADR-0027 §1 / P70), each a separate `editorial_revision` on its own path, keyed by a compound `(atom_type:change_kind)` template (`EditorialStateService::getStagePath`):
...
- **Canonical source not settled:** the Gap Detector needs a canonical atom store to diff; ops#61 (v3 catalog vs ss2 v1 counts reconciliation) is NWP-ADR-0027's own stated prerequisite — decide the canonical 247 before bulk import.