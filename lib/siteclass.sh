...
# NEVER suppress the scan that produces the failure. Concretely, in
# lib/moodle-gate.sh the class is consulted ONLY on the failure path: an
# artifact that passes the ordinary scan passes regardless of class, and an
# artifact that fails can be excused only by `none-stored` backed by evidence
# that itself fails closed on absence, staleness, expiry, a non-zero formation
# row count, or an exceeded member cap.
#
...
siteclass_art9_check() {
    ...
    if [ -n "$allowed" ] && ! printf '%s' " $allowed " | grep -q " $posture "; then
