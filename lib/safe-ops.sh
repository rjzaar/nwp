...
safe_server_status() {
    ...
    local status_output
    status_output=$(ssh -i "$ssh_key" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o BatchMode=yes \
        "${ssh_user}@${ssh_host}" \
...
safe_db_status() {
    ...
    # Check for recent backups (F23: sites/<site>/backups/ with legacy fallback)
    local backup_dir
