...
**Test status:** see the [CI pipelines](../../pipelines) for this repository.

<!--
The three shields.io badges that used to sit here ("Automated Tests 90%",
"Human Verified 1%", "Fully Verified") were removed on 2026-07-26. They read
.badges.json from raw.githubusercontent.com — a mirror this self-hosted repo
does not push to — and the committed .badges.json was generated by hand on
2026-02-02 ("pipeline": {"id": "local"}). The refresh job was allow_failure and
triggered only on files that do not change when test results change. A number
that cannot move is worse than no number; a link to the live pipeline is honest.
See the `update-badges` tombstone comment in .gitlab-ci.yml.
-->
