...
yaml_get_setting() {
    ...
    if [[ -z "$key_path" ]]; then
        echo -e "${RED}Error: Key path is required${NC}" >&2
        return 1
    fi
    ...
    if command -v yq &>/dev/null; then
        local result
        # NOTE: do NOT use `// ""` here — yq treats `false` as nullish, so
        # `.settings.debug // ""` returns "" when the value is literally
        # false. Read the raw value and filter "null" explicitly.
...
yaml_get_secret() {
    local key_path="$1"
    local secrets_file="${2:-.secrets.yml}"
