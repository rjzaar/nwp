...
# Uses the SAMPLE ssd pair contract shipped in pairs/ (sub_stability: uuid —
# the join semantics under test; the real pair's contract is overlay, ops#326).
...
@test "GREEN when nwc holds the uuid AND mdl idnumber == uuid" {
  run bash "$SMOKE_SH" ssd --tier=live --join-uuid=ff1e2312 --nwc-ledger="$LED" --mdl-idnumber=ff1e2312
  [ "$status" -eq 0 ]
...
@test "RED when the Moodle idnumber does not match the uuid (broken join)" {
  run bash "$SMOKE_SH" ssd --tier=live --join-uuid=ff1e2312 --nwc-ledger="$LED" --mdl-idnumber=WRONG
  [ "$status" -ne 0 ]
...
@test "RED when the uuid is absent on the provider (severed identity)" {
  run bash "$SMOKE_SH" ssd --tier=live --join-uuid=NOPE --nwc-ledger="$LED" --mdl-idnumber=NOPE
  [ "$status" -ne 0 ]
...
@test "AMBER (not GREEN) when no Moodle resolver is supplied — half check only" {
  run bash "$SMOKE_SH" ssd --tier=live --join-uuid=ff1e2312 --nwc-ledger="$LED"
  [ "$status" -eq 0 ]
  [[ "$output" == *"half-verified"* ]]
  [ "$(cat "${NWP_PAIR_STATE_DIR}/ssd.live.rag")" = "amber" ]
}

@test "--join without a uuid is refused" {
  run bash "$SMOKE_SH" ssd --tier=live --join
  [ "$status" -ne 0 ]
