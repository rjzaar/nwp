...
EOF
    ...
    #
    # ops#151 F4 — RENAME DETECTION HIDES A SENSITIVE SOURCE PATH.
    # `git diff` enables rename detection by default (git >= 2.9). A rename of
    # a SENSITIVE file to a benign name collapses to a single `R100` entry and
    # `--name-only` prints ONLY the destination — e.g. renaming `lib/auth.sh`
    # (or `.gitlab-ci.yml`, or a live-deploy guard) to `docs/benign.md` shows
    # the gate just `docs/benign.md`, while the pushed tree DELETES the
    # sensitive file. Deleting/relocating a sensitive path IS a sensitive
    # change, so the source must be visible. `--no-renames` forces every
    # rename to appear as delete(old-path) + add(new-path), so the sensitive
    # source path is always in the record stream and gets matched. Verified
    # end-to-end 2026-07-28 on git 2.43.0 (R100 lib/auth.sh -> docs/benign.md).
    gate_tmp="$(mktemp -d)"
