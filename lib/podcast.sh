...
# Generate Castopod environment variables with local storage (no S3/B2)
# Usage: generate_castopod_env_local "domain" "db_password"
generate_castopod_env_local() {
    local domain=$1
    local db_password=$2
    local analytics_salt=$(openssl rand -base64 48 | tr -d '/=+' | cut -c -64)

    cat << EOF
# Castopod Environment Configuration
# Generated by NWP Podcast Setup (Linode-only mode)
...
# Media Storage (Local - stored on VPS)
# Media files served directly from the same domain
# No external storage (B2/S3) required
CP_MEDIA_BASE_URL="https://${domain}"
...
# Analytics (required for Castopod)
...
export -f generate_castopod_env_local
