...
# Delete a site — DELEGATES to delete.sh, the single deletion engine
# (nwp/ops#47). It prints the full impact report (fate manifest) and runs
# its own confirmation; keeping a parallel implementation here is exactly
# how the v2 bugs happened, so don't.
delete_site() {
    local site="$1"
    local config_file="$2"
    local force="${3:-false}"
    ...
    local args=()
    [ "$force" = "true" ] && args+=(-y)
    "$SCRIPT_DIR/delete.sh" "${args[@]}" "$site"
}
...
show_production_dashboard() {
    ...
    while read -r site; do
        ...
        # Backup age
        local backup_age="-"
        # F23 Phase 4: prefer sites/<name>/backups/, fall back to legacy sitebackups/
        local backup_dir
