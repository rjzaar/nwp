...
class ShariffSettingsForm extends ConfigFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach (Element::children($form) as $variable) {
      ...
      if ($variable == 'shariff_services') {
        // Process not checked services.
        $value = array_map(function ($v) {
          return $v['service'] ?: NULL;
        }, $value);
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // Setup the proper service order.
    $ordered_services = [];
    foreach ($settings['services'] as $service) {
      if (!empty($services[$service])) {
        $ordered_services[$service] = $services[$service];
      }
    }
    $ordered_services += $services;

    $form['shariff_hidden'] = [
      '#title' => $this->t('Hide block if Web Share API is supported'),
      '#description' => $this->t('The block will be not visible if Web Share API is supported by browser.'),
      '#type' => 'checkbox',
      '#default_value' => \Drupal::config('shariff.settings')->get('shariff_hidden'),
    ];

    $form['shariff_services'] = [
      '#type' => 'table',
      '#header' => [
        $this->t('Activated services'),
        $this->t('Weight'),
      ],
    ...
    $weight = 0;
    foreach ($ordered_services as $service => $label) {
      // TableDrag: Mark the service row as draggable.
      $form['shariff_services'][$service]['#attributes']['class'][] = 'draggable';
      // TableDrag: set weight, the order is the saved order.
      $form['shariff_services'][$service]['#weight'] = $weight;

      // Add service checkbox.
      $form['shariff_services'][$service]['service'] = [
        '#type' => 'checkbox',
        '#title' => $label,
        '#return_value' => $service,
        '#default_value' => empty($settings['services'][$service]) ? NULL : $service,
        // '#checked' => !empty($services[$service])
      ];

      // TableDrag: Weight column element.
      $form['shariff_services'][$service]['weight'] = [
        '#type' => 'weight',
 ...
 Left empty the page title is used."),
  ...
  /**
   * Get list of all share services.
   *
   * @return array
