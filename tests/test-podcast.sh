...
# NWP CLI relocated podcast.sh into scripts/commands/ (root symlink removed).
PODCAST_SH="scripts/commands/podcast.sh"

run_test "podcast.sh exists" "[ -f $PODCAST_SH ]"
run_test "podcast.sh is executable" "[ -x $PODCAST_SH ]"
run_test "podcast.sh syntax valid" "bash -n $PODCAST_SH"
run_test "podcast.sh has help" "$PODCAST_SH --help >/dev/null 2>&1"
run_test "podcast.sh supports setup command" "$PODCAST_SH --help 2>&1 | grep -q 'setup'"
run_test "podcast.sh supports generate command" "$PODCAST_SH --help 2>&1 | grep -q 'generate'"
run_test "podcast.sh supports deploy command" "$PODCAST_SH --help 2>&1 | grep -q 'deploy'"
run_test "podcast.sh supports teardown command" "$PODCAST_SH --help 2>&1 | grep -q 'teardown'"
run_test "podcast.sh supports status command" "$PODCAST_SH --help 2>&1 | grep -q 'status'"
...
# F23 Phase 8: Linode StackScripts moved to servers/nwpcode/linode/. Legacy fallback retained.
PODCAST_STACK="servers/nwpcode/linode/podcast_server_setup.sh"
[ -f "$PODCAST_STACK" ] || PODCAST_STACK="linode/podcast_server_setup.sh"

run_test "podcast_server_setup.sh exists" "[ -f $PODCAST_STACK ]"
run_test "podcast_server_setup.sh is executable" "[ -x $PODCAST_STACK ]"
run_test "podcast_server_setup.sh syntax valid" "bash -n $PODCAST_STACK"
run_test "StackScript has UDF parameters" "grep -q '<UDF' $PODCAST_STACK"
run_test "StackScript installs Docker" "grep -q 'docker' $PODCAST_STACK"
...
run_test "example.nwp.yml has podcast settings" "grep -q 'type: podcast' example.nwp.yml"
...
run_test "install.sh handles podcast type" "grep -q 'podcast' scripts/commands/install.sh"
...
# Generate files
# NWP CLI: podcast.sh generate writes into its own script directory.
PODCAST_SH_DIR="$(dirname "$PODCAST_SH")"
if "$PODCAST_SH" generate "$TEST_DOMAIN" >/dev/null 2>&1; then
    # Find the generated directory next to podcast.sh.
    GEN_DIR="$(ls -td "$PODCAST_SH_DIR"/podcast-setup-* 2>/dev/null | head -1 || true)"
    ...
    if [ -n "${GEN_DIR:-}" ] && [ -d "$GEN_DIR" ]; then
        ...
        TESTS_RUN=$((TESTS_RUN + 1))
        TESTS_FAILED=$((TESTS_FAILED + 1))
        FAILED_TESTS+=("Generated directory not found")
    fi
else
